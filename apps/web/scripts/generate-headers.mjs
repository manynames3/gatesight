...
for (const value of [api, upload, cognito]) {
...
const headers = `/*
  ...
  Content-Security-Policy: default-src 'self'; base-uri 'none'; object-src 'none'; frame-ancestors 'none'; form-action 'self' ${upload}; script-src 'self'; style-src 'self'; img-src 'self' blob: data:; media-src 'self' blob:; connect-src 'self' ${api} ${upload} ${cognito}; font-src 'self'; upgrade-insecure-requests
